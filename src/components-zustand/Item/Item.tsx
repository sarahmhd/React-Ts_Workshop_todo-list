...
const Item: React.FC<ItemProps> = ({ item, index }) => {
  ...
  const changeItemDone = useItemsStore((store) => store.changeItemDone);
  const deleteItem = useItemsStore((store) => store.removeItem);
  const editItem = useItemsStore((store) => store.editItem);
